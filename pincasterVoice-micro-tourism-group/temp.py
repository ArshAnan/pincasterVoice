...
dataset = pd.read_csv("pincasterVoice-micro-tourism-group\google_maps_restaurants(cleaned).csv")
index_col="Name"

def findPointsOfInterestInRadius(lat, lon, tolerance, dataset):
    """tolerance in meters"""
    ...
    #needs testing
    while poiList.length == 0:
        for location in dataset.loc[:, ["Name", "Lat", "Lon"]]:
            if (location[1] < latMax and location[1] > latMin and location[2] < lonMax and location[2] > lonMin):
                poiList.append(location)
